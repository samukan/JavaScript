...
const picarray = document.querySelector('#pictures');
...
picArray.forEach((pic) => {
  ...
  image.alt = pic.caption;
  figure.appendChild(image);

  article.addEventListener("click", () =>{
    const imaged = document.createElement('img');
    imaged.src = pic.image.large;
    imaged.alt = pic.caption;
    const modalImage = dialog.querySelector('img');
  if (modalImage) {
    dialog.removeChild(modalImage);
  }
    dialog.appendChild(imaged);
    dialog.showModal();
  })

  document.querySelector('span').addEventListener("click", () => {
    dialog.close();
    const imaged = document.querySelector('img');
    imaged.src = "";
    imaged.alt = "";
    dialog.appendChild(imaged);
  });
